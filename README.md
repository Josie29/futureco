...
- **Then the model is constrained structurally, not by instruction.** `filter.run` takes a typed `Composition` so the unsafe call cannot be written; `Op` has no waive verb and a test asserts it; another test **walks the AST of every module under `plan/`** and fails if one imports `anthropic`.
