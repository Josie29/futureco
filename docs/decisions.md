...
3. **Clamped `estimated_rep_duration` in the estimator rather than editing the fixture.** The field is documented as minutes per rep and holds up across most of the catalog — 3 × 10 at 0.3 is nine minutes of work, and her completed sessions ran 26–31 minutes for three movements. It does not hold at the top of the range: `Jump Rope - Single-Leg` carries **1.9** (114 seconds for one skip) and `Push-Up to Knee-Drive` **1.2**. Those rows read like time per set or per bout, and no field distinguishes the two.

   Unclamped, the planner priced a single tricep extension at 25 of a 50-minute session. The estimate now clamps to 3–21 seconds a rep, a band a coach would recognise. The clamp lives in `estimateMinutes` rather than in the data because — unlike the empty `joints_loaded` lists above — the underlying value is *ambiguous*, not *wrong*, and rewriting it would assert a reading of the field the dataset doesn't support. **This clamp currently only exists in the frontend mock; the real estimator needs its own decision here** — see `docs/mock-notes.md`.
