...
4. **Contraindication rules are keyed by clinical condition, not by injury.** *"Patellofemoral pain contraindicates plyometrics"* is knowledge about a condition and applies to the next member who presents with it; keying it to `inj_knee_left` would make it member-specific, which is wrong. So `injuries[]` gains an explicit `condition` — the same POC shortcut as `Goal.targets`, standing in for LLM extraction from the free-text `notes` — and `data/authored/contraindications.json` is keyed by that condition. The SNOMED code lives on the condition rather than the injury, since the condition is the clinical entity SNOMED names, and it is the side-neutral concept (`430725003 Patellofemoral stress syndrome`) rather than the left-knee-specific one, or the rules would not transfer.
